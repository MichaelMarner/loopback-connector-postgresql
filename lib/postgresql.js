...
PostgreSQL.prototype.executeSQL = function(sql, params, options, callback) {
  ...
  if (self.settings.debug) {
    if (params && params.length > 0) {
      self.debug(g.f('SQL: %s\nParameters: %j', sql, params));
    } else {
      self.debug(g.f('SQL: %s', sql));
    }
  }
  ...
  function executeWithConnection(connection, done) {
    connection.query(sql, params, function(err, data) {
      ...
      if (self.settings.debug && data) self.debug(g.f("%j", data));
      if (done) {
        process.nextTick(function() {
  ...
  if (transaction && transaction.connection &&
    ...
    debug(g.f('Execute SQL within a transaction'));
  ...
  switch(operator) {
    ...
    case 'regexp':
      if (operatorValue.global)
        console.warn(g.f('PostgreSQL regex syntax does not respect the `g` flag'));

      if (operatorValue.multiline)
        console.warn(g.f('PostgreSQL regex syntax does not respect the `m` flag'));
